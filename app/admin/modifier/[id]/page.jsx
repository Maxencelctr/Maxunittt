...
export default function ModifierProduit({ params }) {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const produitId = resolvedParams.id
      setId(produitId)
      ...
      if (data) {
        setForm(data)
        if (data.photos) {
          const tabPhotos = data.photos.split(',')
          const photosInitiales = ['', '', '', '', '', '']
          tabPhotos.forEach((url, index) => {
            if (index < 6) photosInitiales[index] = url
          })
          setPhotos(photosInitiales)
        }
        if (data.couleur) {
          setCouleurs(data.couleur.split(','))
        }
      }
  ...
  const handleSubmit = async (e) => {
    ...
    const { id: produitId, created_at, ...formSansId } = form
  ...
  if (!form) return <div className="px-8 py-10 text-[#8C7B6B]">Chargement...</div>
  ...
  return (
    <div className="px-8 py-10 max-w-2xl mx-auto">
      ...
      <h1 className="text-3xl font-bold text-[#3D2B1F] mb-8">Modifier le produit</h1>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select name="marque" value={marques.find(m => m.nom.toLowerCase() === (form.marque || '').toLowerCase()) ? form.marque : '__nouvelle__'} onChange={handleChange} required className="w-full border border-[#E8DFD0] rounded-xl px-4 py-3 text-sm text-[#3D2B1F] bg-transparent mt-1 outline-none focus:border-[#3D2B1F]">
            ...
            {(!marques.find(m => m.nom.toLowerCase() === (form.marque || '').toLowerCase()) || form.marque === '__nouvelle__') && (
              <input
                type="text"
                placeholder="Nom de la nouvelle marque"
                value={form.marque === '__nouvelle__' ? '' : form.marque || ''}
                onChange={async e => {
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <select name="categorie" value={categories.find(c => c.slug === form.categorie) ? form.categorie : '__nouvelle__'} onChange={handleChange} className="w-full border border-[#E8DFD0] rounded-xl px-4 py-3 text-sm text-[#3D2B1F] bg-transparent mt-1 outline-none focus:border-[#3D2B1F]">
            ...
            {(!categories.find(c => c.slug === form.categorie) || form.categorie === '__nouvelle__') && (
              <input
                type="text"
                placeholder="Nom de la nouvelle catégorie"
                value={form.categorie === '__nouvelle__' ? '' : form.categorie || ''}
                onChange={async e => {
                  const nom = e.target.value
        ...
        <div>
          ...
          <select name="disponible" value={form.disponible} onChange={e => setForm(prev => ({ ...prev, disponible: e.target.value === 'true' }))} className="w-full border border-[#E8DFD0] rounded-xl px-4 py-3 text-sm text-[#3D2B1F] bg-transparent mt-1 outline-none focus:border-[#3D2B1F]">
            <option value="true">Disponible</option>
            <option value="false">Vendu</option>
          </select>
        </div>
